...
export default function Paso1({ alSiguiente }) {
  ...
  const manejarSiguiente = () => {
    if (alSiguiente) alSiguiente({ nombre, correo, telefono, contrasena, revalidar, rol, archivo });
  };
    ...
    <div style={estilos.fondo}>
      <div style={estilos.tarjeta}>
        ...
        <div style={estilos.filaPasos}>
          {["Paso 1", "Paso 2", "Paso 3"].map((etiqueta, i) => (
            <div key={etiqueta} style={estilos.itemPaso}>
              <div style={{ ...estilos.barraPaso, background: i === 0 ? "#1a2332" : "#dde4ef" }} />
              <span style={{ ...estilos.etiquetaPaso, color: i === 0 ? "#1a2332" : "#aaa" }}>{etiqueta}</span>
            </div>
          ))}
        </div>
        ...
        <div style={estilos.contenedorIcono} onClick={() => referenciaEntrada.current.click()}>
          {archivo ? (
            <span style={estilos.nombreArchivo}>{archivo.name}</span>
          ) : (
            <svg
              width="64"
              height="57"
        ...
        <div style={estilos.filaRol}>
          {["trabajador", "cliente"].map((r) => (
            <label key={r} style={estilos.etiquetaRol} onClick={() => setRol(r)}>
              <div
                style={{
                  ...estilos.puntoRol,
        ...
        <button style={estilos.boton} onClick={() => navegar("/paso2")}>
          Siguiente
        </button>
      </div>
    </div>
